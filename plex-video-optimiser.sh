...
OUTPUT_FILE_NAME=$(echo ${OUTPUT_FILE_NAME} | sed 's/[-\ \.]*\(|AMZN\|Disney\|Vimeo\|WEB-DL\|WEBRip\)//g')
OUTPUT_FILE_NAME=$(echo ${OUTPUT_FILE_NAME} | sed 's/[-\ \.]*\(REPACK\|Remux\|REMUX\|RoSubbed\)//g')
OUTPUT_FILE_NAME=$(echo ${OUTPUT_FILE_NAME} | sed 's/[-\ \.]*\(AC3\|AAC2\.0\|Atmos\|DTS\|HD[-\ \.]MA\|DD+\|DTSX\|TrueHD\|5.1\|6.1\|7\.1\)//g')
OUTPUT_FILE_NAME=$(echo ${OUTPUT_FILE_NAME} | sed 's/[-\ \.]*\(ETRG\|GOLDIES\|TrollUHD\|FraMeSToR\|playBD\|MovietaM\|HDMaN\|BLUEBIRD\|TrollUHD\|MTeam\)//g')
